...
export async function ReadByte(fd: any): Promise<number> {
...
export async function ReadBytes(fd: any, length: number = 1, needBuffer: boolean = false): Promise<string | Buffer> {
...
export async function ReadInt8(fd: any): Promise<number> {
    const buffer: Buffer = Buffer.allocUnsafe(1); // 创建一个1字节的Buffer
    await fd.read(buffer, 0, 1, null);
    return buffer.readInt8(0); // 使用小端序解析无符号整数
}

// 流中读取下一个两字节有符号整数，并将其作为一个16位整数返回
// 按照二进制方式从流中读取2个字节，并将其解释为一个16位的有符号整数
export async function ReadInt16(fd: any): Promise<number> {
    const buffer: Buffer = Buffer.allocUnsafe(2); // 创建一个2字节的Buffer
    await fd.read(buffer, 0, 2, null);
    return buffer.readInt16LE(0); // 使用小端序解析无符号整数
}

// 从流中读取下一个四字节有符号整数，并将其作为一个32位整数返回
// 按照二进制方式从流中读取4个字节，并将其解释为一个32位的有符号整数
export async function ReadInt32(fd: any): Promise<number> {
    const buffer: Buffer = Buffer.allocUnsafe(4); // 创建一个4字节的Buffer
    await fd.read(buffer, 0, 4, null);
    return buffer.readInt32LE(0); // 使用小端序解析无符号整数
}
...
function readNextByte(fd: any): Promise<string[]> {
    return new Promise((res, rej) => {
        ...
        let length = 0;
        // 读取文件流的游标
        const readNext = async (fd: any) => {
            await fd.read(bytes, length, 1, null);
            if (bytes[length] === 0) {
...
// <a>123<b>456</b></a>
export async function readDict(fd: any, strings: string[]): Promise<any> {
    // 读取字典的子节点数量
...
export async function readData(fd: any, strings: string[], type: number, length: number) {
    switch(type) {
        case typeEnum.Dict:
...
export async function bXmlReader(fd: any) {
    // 检查文件头部，是否是xml文件
